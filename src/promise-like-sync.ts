...
import * as PromiseLike from "./promise-like";

class JustValue<X> implements PromiseLike<X> {
  constructor(public value: X) {}
  then<Y>(
    onfulfilled?: ((value: X) => Y | PromiseLike<Y>) | null
  ): JustValue<Y> {
    return JustValue.lift(onfulfilled!(this.value));
  }
  static lift<Y>(y: Y | PromiseLike<Y>): JustValue<Y> {
    if (y instanceof JustValue) {
      return y;
    } else if (PromiseLike.isPromiseLike(y)) {
      throw "nope";
    }
    return new JustValue(y);
  }
}

export interface SyncFactoryConfig extends PromiseLike.FactoryConfig {}
...
export class Generator<T> extends PromiseLike.Generator<T> {
  constructor(readonly func: (seq: number) => T) {
    super(func, JustValue.lift);
  }
}
export class Derived<TOwner, TProperty> extends PromiseLike.Derived<
  TOwner,
  TProperty
> {
  ...
  public build(owner: TOwner, seq: number): TProperty {
    return (this.buildInner(owner, seq) as JustValue<TProperty>).value;
  }
}
